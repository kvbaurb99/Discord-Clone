...
export default function AddFriends({username, userId, usersData}) {
    ...
    const findUser = usersData ? usersData.filter(user => user.username === idReg) : null;
...
<div className='w-full h-full flex flex-col text-gray-300'>
    <div className='w-full p-8 flex flex-col border-b border-gray-400/20'>
      ...
      {alert ? <div className="absolute top-0 left-0 right-0 mx-auto w-1/4 h-1/10 bg-blue-600 rounded-bl-xl rounded-br-xl flex justify-center items-center error">
        <p className="text-white text-center font-bold">{alert}</p>
        </div> : null}
      </form>
    </div>
</div>
